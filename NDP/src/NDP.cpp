...
// Direct logging system - no buffering, immediate output
static bool log_enabled = false;  // DISABLED: Set to false to disable all NDP logging
...
// Direct logging macros - DISABLED BY DEFAULT (empty macros for performance)
#define LOG_NDP(msg) do { \
    if (log_enabled) { \
        Serial.print("[NDP:"); \
        Serial.print(millis()); \
        Serial.print("] "); \
...
#define LOG_NDP_BUF(msg, buf, len) do { \
    if (log_enabled) { \
        Serial.print("[NDP:"); \
        Serial.print(millis()); \
        Serial.print("] "); \
...
// SPI logging only after CS changes to reduce noise
#define LOG_SPI_CS_LOW() do { \
    if (log_enabled) { \
        Serial.print("[NDP:"); \
        Serial.print(millis()); \
        Serial.print("] "); \
...
#define LOG_SPI_TRANSFER(tx, rx, len) do { \
    if (log_enabled) { \
        Serial.print("[NDP:"); \
        Serial.print(millis()); \
        Serial.print("] "); \
...
// Made public for debugging (removed static)
struct syntiant_ndp120_tiny_device_s _ndp;
struct syntiant_ndp120_tiny_device_s *ndp = &_ndp;
...
int NDPClass::spiTransfer(void *d, int mcu, uint32_t address, void *_out, void *_in, unsigned int count) {
  // Reduced logging - only log CS changes and key events
  mtx.lock();
  LOG_NDP("SPI:LOCKED");
  ...
  if (mcu) {
    ...
    if (out) {
      ...
      uint8_t read_cmd = 0x80 | NDP120_SPI_MADDR(0);
...
int NDPClass::load(const char* fw, int bl) {
  ...
  LOG_NDP_DEC("LOAD:CHUNK", chunk_size);
  
  char *file_data = (char *) malloc(sizeof(char) * chunk_size);
  if (!file_data) {
      LOG_NDP("LOAD:ERR:MALLOC");
  ...
  LOG_NDP_DEC("LOAD:SIZE", file_length);
  
  unsigned int total_loaded = 0;
  while (chunk_size) {
    LOG_NDP_DEC("LOAD:CHUNK:SIZE", chunk_size);
    fread(file_data, 1, chunk_size, package);
    LOG_NDP_BUF("LOAD:CHUNK:DATA", (uint8_t*)file_data, min(chunk_size, 16));
    s = syntiant_ndp120_tiny_load(ndp, file_data, chunk_size);
    LOG_NDP_DEC("LOAD:CHUNK:RET", s);
    total_loaded += chunk_size;
    LOG_NDP_DEC("LOAD:TOTAL", total_loaded);
  ...
  fclose(package);
  loaded++;
  LOG_NDP_DEC("LOAD:COUNT", loaded);
  if (file_data) {
    free(file_data);
  }
  if (file_length) {
    LOG_NDP("LOAD:ERR:INCOMPLETE");
...
int NDPClass::begin(const char* fw1) {
  // Logging disabled by default - only enable if explicitly requested
  // start_log_thread(); // DISABLED: Only enable if begin(fw1, true) is called
  LOG_NDP("BEGIN:START");
  memset(ndp, 0, sizeof(*ndp));
  
  // Prepare for SPI
  LOG_NDP("GPIO:PORSTB:OUT");
  pinMode(PORSTB, OUTPUT);
  LOG_NDP("RST:LOW");
  digitalWrite(PORSTB, LOW);
  LOG_NDP_DEC("DELAY:100", 100);
  delay(100);
  LOG_NDP("RST:HIGH");
  ...
  LOG_NDP("SPI:BEGIN");
  SPI1.begin();
  LOG_NDP_DEC("SPI:SPEED", spi_speed_initial);
...
void NDPClass::enableLogging(bool enable) {
    log_enabled = enable;
}

void NDPClass::disableLogging() {
    log_enabled = false;
}

bool NDPClass::isLoggingEnabled() {
    return log_enabled;
}

// begin with logging control
int NDPClass::begin(const char* fw1, bool enable_logging) {
    // Set logging state before initialization
